...
import org.jumpmind.symmetric4.job.IWorker;
import org.jumpmind.symmetric4.job.manage.SyncTriggersWorker;
...
public abstract class AbstractSymmetricEngine implements ISymmetricEngine, IApplicationContext {
    ...
    protected Map<Class<?>, IWorker> workers = new HashMap<Class<?>, IWorker>();
    ...
    protected void init(boolean registerEngine) {
        ...
        long cacheTimeout = properties.getLong(ParameterConstants.CACHE_TIMEOUT_TABLES_IN_MS);
    ...
    public void start() {
        ...
        for (IWorker worker : workers.values()) {
            worker.start();
        }
        ...
        if (parameters.is(ParameterConstants.AUTO_SYNC_TRIGGERS_AT_STARTUP)) {
            getWorker(SyncTriggersWorker.class).scheduleNow();
        }
    }
